...
test('card REST request carries the original identity token',async()=>{
 let sent;
 await sendBoundOperation({...op(),method:'set',creationGuard:true,payload:{notes:'fixture'}},{currentIdentity:identity,currentUser:()=>({uid:'one',getIdToken:async()=> 'original-token'}),projectId:'mci2-test',databaseURL:cfg.firebase.databaseURL},async(url,options)=>{if(!options.method)return Response.json(null,{headers:{etag:'"empty"'}});sent={url:new URL(url),options};return new Response('{}');});
