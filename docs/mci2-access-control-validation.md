...
3. 관리자 종료/보관/재개와 두 단말의 동시 수정 흐름을 종합 검증한다. 현재 종료 코드는 백업 조회·저장과 원본 삭제가 별도 요청이어서 그 사이 들어온 수정이 백업에서 빠질 수 있다. 서비스 전환 전에 종료 절차를 보강해야 한다. 같은 필드 충돌을 자동 병합하지 않는다.
...
이번 변경에서 저장 회귀 30개, 세션 등 41개, RTDB 에뮬레이터 85개가 통과했다(합계 156개). 신규 검사는 오프라인 보관, 저장공간 부족, 토큰 갱신 도중 계정 변경, 재전송 순서와 타 관서·읽기 전용·종료 재난 쓰기 차단을 포함한다. SW 버전은 v97-field-outbox다. 이 변경은 Git 브랜치에만 반영하며 위 Cloudflare 후보는 이전 v96 버전이다. 실제 서비스 트래픽은 변경하지 않았다.
