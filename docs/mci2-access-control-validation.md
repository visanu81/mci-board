...
최신 Cloudflare 후보 버전은 `cf953653-b910-4d26-96b0-4ca86acf1567`(태그 safe-close)다. 서버 Secret과 코드를 `versions upload`로 함께 업로드했으며 서비스 트래픽은 전환하지 않았다. mci2 서비스 버전은 기존 `939f0bb6-e85c-4c34-8680-b9db6cad5cf2`를 유지한다. 후보 URL의 브라우저 Origin은 허용하지 않으므로 현장 로그인 주소로 안내하지 않는다.
